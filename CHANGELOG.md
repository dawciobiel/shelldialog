...
### Added
- `WizardSummaryStep` now supports an optional intro line above grouped summary sections.
...
- `FileDialog` metadata preview now shows compact `rwx` permissions.
...
- `WizardDialog<T>` for multi-step flows with shared context, built-in text/password/directory/file/summary steps, and typed final result mapping.
