...
When metadata preview is enabled, `FileDialog` also renders a small panel for the currently focused entry, including its label, type, path, human-readable file size, last modified timestamp, and compact `rwx` permissions when available.
